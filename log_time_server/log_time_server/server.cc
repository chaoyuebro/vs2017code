...
void comServer::dealMessage(TcpConnectionPtr &conn, const string& msg)
{
	...
	if (msg.size() >= headerLen)
	{
		...
		if (cmdBuf.find(' ') >0)
		{
			cmd = cmdBuf.substr(0, cmdBuf.find(' '));
		}
		else
		{
			cmd = cmdBuf;
		...
		if (formatBuf.find(' ') > 0)
		{
			format = formatBuf.substr(0, formatBuf.find(' '));
		}
		else
		{
			format = formatBuf;
		...
		if (userNameBuf.find(' ') > 0)
		{
			userName = userNameBuf.substr(0, userNameBuf.find(' '));
		}
		else
		{
		...
		muduo::string keyBuf(msg.substr(44, 4));
		muduo::string key;

		if (keyBuf.find(' ') > 0)
		{
			key = keyBuf.substr(0, userNameBuf.find(' '));
		}
		else
		{
			key = keyBuf;
		...
		if (cmd == "RQUPJ"&&format == "GZIP")
		{
			...
			if (sqlConn == nullptr)
			{
				...
				sqlConn->mysqlInsert(userName,body,key,side,res);
			}
			codec_.send(get_pointer(conn),"RSUPJ","TXT", userName,key,side,res);
		}
		else if (cmd == "RQDPJ")//等于0加get表示后面没跟数据,并且需要返回数据
		{
			string sqlQueryRes;
			shared_ptr<MysqlConnection> sqlConn = mysqlPool_->getOneConnect();
			sqlConn->mysqlQuery(userName, key,side,sqlQueryRes);
			...
			{
				format = "TXT";
				codec_.send(get_pointer(conn), cmd, format,userName,key,side,sqlQueryRes);

			}
			else
			{
				format = "GZIP";
				codec_.send(get_pointer(conn), cmd, format, userName,key,side,sqlQueryRes);//将查询结果作为字符串返回，并且加上getRes头
			}
		}
		else if (cmd != "HBET")
		{
