...
void MysqlConnection::mysqlInsert(const string&userName, const string &content,const string& key,const string& side,string &res)
{
	int t; 
	...
	stringstream ss;
	sprintf(query, "REPLACE INTO %s(userName,\`key\`,side,content) VALUES (\"%s\",\"%s\",\"%s\",\'",TABLE_NAME,userName.c_str(),key.c_str(),side.c_str());
...
void MysqlConnection::mysqlQuery(const string &userName, const string& key,const string& side,string &getRes)
{
	int t;
	...
	MYSQL_RES *res;
	MYSQL_ROW row;
	sprintf(query, "%s %s where userName = \"%s\" AND \`key\`=\"%s\" AND side=\"%s\"", head, TABLE_NAME, userName.c_str(),key.c_str(),side.c_str());
